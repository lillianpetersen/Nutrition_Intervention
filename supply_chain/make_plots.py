...
loopvar = ['AllarOpti_SHIPCOST','AllarOpti_impexp','AllarOpti_strtup','AllarOpti_truckfactor','LocalOpti_SHIPCOST','AllarOpti_impexp','AllarOpti_strtup','AllarOpti_truckfactor']
...
for i in range(len(optiLevel)):
	for j in range(len(loopvar)):
		File = optiLevel[i]+'_'+loopvar[j]
		...
		countrycostedS = np.load(wdvars+'Scountrycosted.npy')
		countrycosted = np.load(wdvars+'countrycosted.npy')
		
		i=-1
		for s in np.arange(factor[j],maxs[j],factor[j]):
			s=np.round(s,2)
			i+=1
			countriesWfactories=[]
			factorySize=[]
		
			f = open(wddata+'results/'+File+'/'File+str(s)+'.csv')
			k=-1
			j=-1
			for line in f:
				k+=1
