...
# distanceArray=np.zeros(shape=(definenumberherepls,definenumberherepls))
# distanceDictionary={}
# counter=0
...
# numpy.savetxt(wddata + "travel_time/capitaldistanceArray.csv", distanceArray, delimiter=",")
...
f=open(wddata+'foodstuffs/transportationcostsarray.csv','r')
i=-1
for line in f:
    i+=1
    tmp=line.split(',')
...
for i in range(len(countrycosted)):
    for j in range(len(subsaharancountry)):
        if countrycosted[i]==subsaharancountry[j]:
            ...
            indexedscp[j]=scplusprice[i]
            

##create arrays
rutfarray = np.zeros(shape=(43,43))
rusfarray = np.zeros(shape=(43,43))
scparray = np.zeros(shape=(43,43))
samarray=np.zeros(shape=(43,43))
mamarray=np.zeros(shape=(43,43))
for i in range(len(indexedrutf)):
    rutfarray[i,:]=indexedrutf[i]
    rusfarray[i,:]=indexedrusf[i]
    scparray[i,:]=indexedscp[i]
    samarray[i,:]=indexedSAM
    mamarray[i,:]=indexedMAM

numpy.savetxt(wddata + "optiarrays/rutfarray.csv", rutfarray, delimiter=",")
numpy.savetxt(wddata + "optiarrays/rusfarray.csv", rusfarray, delimiter=",")
numpy.savetxt(wddata + "optiarrays/scparray.csv", scparray, delimiter=",")

numpy.savetxt(wddata + "optiarrays/samarray.csv", samarray, delimiter=",")
numpy.savetxt(wddata + "optiarrays/mamarray.csv", mamarray, delimiter=",")

#cost dabber
rutfcostarray=np.zeros(shape=(22,43))
for i in range(len(subsaharancountry)):
    if(indexedrutf[i]!=0):
        for j in range(len(indexedSAM)):
            rutfcostarray[int(convertarray[i]),j]=indexedrutf[i]*53*11.66+53*11.66*92/1000000*transportcostArray[i,j]
...
numpy.savetxt(wddata + "foodstuffs/rutfcostarray.csv", rutfcostarray, delimiter=",")
 
costarrayfixed=np.zeros(shape=(20,43))
f=open(wddata+'foodstuffs/costarray.csv','r')
i=-1
for line in f:
    i+=1
    tmp=line.split(',')
...
numpy.savetxt(wddata + "foodstuffs/subsaharan_costarray.csv", costarrayfixed, delimiter=",")

costarrayfixed=np.zeros(shape=(20,43))
f=open(wddata+'foodstuffs/subsaharan_costarray.csv','r')
i=-1
for line in f:
    i+=1
    tmp=line.split(',')
...
# f=open(wddata+'foodstuffs/rutfarray.csv','w')
# for i in range(len(i)):
#     f.write(+'\n')
# f.close()


f=open(wddata+'population/rutfarray.csv','w')
for i in range(len(i)):
    f.write(+'\n')
f.close()
...
# subsaharancapital1=[]
# subsaharancountry1=[]
# f=open(wddata+'population/countriesandcapitals.csv','r')
# i=-1
# for line in f:
#     i+=1
#     tmp=line.split(',')
#     subsaharancountry1.append(tmp[0])
#     subsaharancapital1.append(tmp[1][:-1])
#     
# ###bycountry
# 
# subsaharancountry=[]
# subsaharancapital=[]
# indexedwasting1=np.zeros(shape=47)
# indexedSAM1=np.zeros(shape=47)
# indexedstunting1=np.zeros(shape=47)
# indexedMAM1=np.zeros(shape=47)
# 
# 
...
#         if subsaharancountry1[j]==tmp[0]:
#             indexedwasting1[j]=float(tmp[1])
#             indexedSAM1[j]=float(tmp[2])
#             indexedMAM1[j]=float(tmp[3])
#             indexedstunting1[j]=float(tmp[4])
# 
# f=open(wddata+'population/CAPITALVERSIONcasenumbers.csv','w')
# for i in range(len(subsaharancountry1)):
#     f.write(str(subsaharancountry1[i]) +','+ str(indexedwasting1[i]) +','+ str(indexedSAM1[i]) + "," + str(indexedMAM1[i]) + "," + str(indexedstunting1[i])+','+ str(subsaharancapital1[i])+'\n')
# f.close()
